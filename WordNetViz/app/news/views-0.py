...
def news():
    ...
    if form.validate_on_submit():
        ...
        if rs:
            for r in rs:
                news.append({'title': r.title, 'date': r.newsdate,'contents':r.contents})
                #print(r.newsdate)
            ...
            news.append({'title':'none', 'date': '0000-00-00','contents':'no thus news'})
...
def wordcloud(texts):
    ...
    for seg in ci:
        if tf[seg] < 5 or len(seg) < 2 or seg in stopword or '一' in seg or '\r\n' in seg:
            tf.pop(seg)
    print(tf)
