...
def getUrl(url):
    #url="http://www.cuc.edu.cn/zcyw/12110.html"
    try:
        r = re.get(url)
        soup = BeautifulSoup(r.text, 'html.parser')
...
def saverec(url, ntitle, nfrom, ndate, ncount, ncontent):
    ...
    cursor = db.cursor()
    try:
        cursor.execute("select * from cucnews where title='%s'"%(ntitle))
        rs=cursor.fetchone()
        if rs:
            print(rs[0],"Updated!")
            sqlUpdate = "update cucnews set newscount ='%s' where title = '%s'"
            cursor.execute(sqlUpdate%(ncount,rs[1]))  # 像sql语句传递参数
            # 提交
            db.commit()
        else:
            cursor.execute(
